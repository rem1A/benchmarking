...
int main(int argc, char *argv []){
	...
	if (1 == restart_id) {
		read_cp(rw_ranks, &iter, &rw_location, image, Ne*sizeof(fp));
	}
    // primary loop [ main loop , iter(Index) , image(WAR)]
    for (; iter<niter; iter++){										// do for the number of iterations input parameter
		...
		if (0 == restart_id && iter == 2) {
			write_cp(rw_ranks, iter, image, Ne * sizeof(fp));
			exit(0);
		}
	}
	...
	if (restart_id == 2) {
		...
		for (i = 0; i < Nr; i++) {
			for (j = 0; j < Nc; j++) {
				fprintf(fid, "%d ", (int)image[j * Nr + i]);
			}
			sprintf(fid, "\n");
		}
	}
