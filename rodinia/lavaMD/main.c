...
void write_cp(int rank, int iter, void* data, int size) {
        ...
        sprintf(filename, "cpk_directory/check_%d_%d", rank, iter);
         ...
         FILE* wr_fp = fopen(filename, "ab");
         assert(NULL != wr_fp);
         fwrite(&size, sizeof(int), 1, wr_fp);
         fwrite(data, size, 1, wr_fp);
         fclose(wr_fp);
 
         sprintf(filename, "cpk_directory/tmp_%d", rank);
         wr_fp = fopen(filename, "wb");
		 assert(NULL != wr_fp);
         fwrite((char*)&iter, sizeof(int), 1, wr_fp);
 ...
 void read_cp(int rank, int* iter, int* location, void* data, int size) {
        ...
        char filename[64];
        sprintf(filename, "cpk_directory/tmp_%d", rank);
        FILE* wr_fp = fopen(filename, "rb");
        assert(NULL != wr_fp);
        fseek(wr_fp, 0, SEEK_SET);
        fread((char*)&local_iter, sizeof(int), 1, wr_fp);
        printf("local_iter: %d\n", local_iter);
        ...
        wr_fp = fopen(filename, "rb");
         assert(NULL != wr_fp);
        fseek(wr_fp, *location, SEEK_SET);
        fread(&size, sizeof(int), 1, wr_fp);
        *location += sizeof(int);
        fread(data, size, 1, wr_fp);
        *location += size;
        fclose(wr_fp);
		...
		char *argv [])
{
	...
	for(dim_cpu.cur_arg=1; dim_cpu.cur_arg<argc; dim_cpu.cur_arg++){
		...
		// unknown
			...
			if (strcmp(argv[dim_cpu.cur_arg], "-wr") == 0) {
				if (argc >= dim_cpu.cur_arg + 1) {
					// check if value is a number
					if (isInteger(argv[dim_cpu.cur_arg + 1]) == 1) {
						restart_id = atoi(argv[dim_cpu.cur_arg + 1]);
						printf("restart_id == %d\n", restart_id);
						dim_cpu.cur_arg = dim_cpu.cur_arg + 1;
					}
					// value is not a number
					else {
						printf("ERROR: Value to -wr parameter in not a number\n");
						return 0;
					}
				}
				else {
					printf("ERROR: Missing value to -wr parameter\n");
					return 0;
				}
			}
			else {
	...
	for(i=0; i<dim_cpu.space_elem; i=i+1){
		...
		rv_cpu[i].v = 0.2;			// get a number in the range 0.1 - 1.0
		rv_cpu[i].x = 0.3;			// get a number in the range 0.1 - 1.0
		rv_cpu[i].y = 0.5;			// get a number in the range 0.1 - 1.0
		rv_cpu[i].z = 0.7;			// get a number in the range 0.1 - 1.0
	}
	...
	for(i=0; i<dim_cpu.space_elem; i=i+1){
		...
		qv_cpu[i] = 0.8;			// get a number in the range 0.1 - 1.0
	}
	printf("fv_cpu == %zu\n", sizeof(*fv_cpu));
	printf(" space _mem %d,space_elem %d\n",dim_cpu.space_mem,dim_cpu.space_elem);
	// output (forces)
