      ...
      char *argv[] )
{
   ...
   while (arg_index < argc)
   {
      ...
      {
          if(strcmp(argv[arg_index], "-wr") == 0){
              arg_index++;
              restart_id = atoi(argv[arg_index++]);
          }
          else
            arg_index++;
      }
   ...
   if (problem_id == 2)
   {
      ...
      int wr_location = 0;
      if(restart_id == 2){
          printf(" init version result========================================\n");
      }

      if(1 == restart_id){
          read_cp(wr_ranks,&j,&wr_location,&diagonal,sizeof(HYPRE_Real));
          read_cp(wr_ranks,&j,&wr_location,&cum_nnz_AP,sizeof(HYPRE_Real));
          read_cp(wr_ranks,&j,&wr_location,&cum_num_its,sizeof(HYPRE_Real));
          read_cp(wr_ranks,&j,&wr_location,&hypre__global_error,sizeof (HYPRE_Int));
          read_cp(wr_ranks,&j,&wr_location,&final_res_norm,sizeof (HYPRE_Real));
      }
      for (; j < time_steps; j++)
      {
         ...
         if(0 == restart_id){
             if(j == 5){
                 write_cp(wr_ranks,j,&diagonal,sizeof(HYPRE_Real));
                 write_cp(wr_ranks,j,&cum_nnz_AP,sizeof(HYPRE_Real));
                 write_cp(wr_ranks,j,&cum_num_its,sizeof(HYPRE_Real));
                 write_cp(wr_ranks,j,&hypre__global_error,sizeof(HYPRE_Int));
                 write_cp(wr_ranks,j,&final_res_norm,sizeof(HYPRE_Real));
                 exit(0);
             }
         }
      }
