...
{   float ce, cw, cn, cs, ct, cb, cc;
    ...
    int rw_location = 0;
    if (1 == restart_id) {
        read_cp(rw_ranks, &i, &rw_location, tOut, nx*ny*nz*sizeof(float));
        read_cp(rw_ranks, &i, &rw_location, tIn, nx * ny * nz * sizeof(float));
    }
    do{///main loop
        for(z = 0; z < nz; z++)
            for(y = 0; y < ny; y++)
        ...
        if (0 == restart_id) {
            if (i == 2) {
                write_cp(rw_ranks, i, tIn, nx * ny * nz * sizeof(float));
                write_cp(rw_ranks, i, tOut, nx * ny * nz * sizeof(float));
                exit(0);
            }
        }
